...
function MenuPageInner() {
  ...
  // (only starts a new session if it isn't already occupied —
  // this way refreshing the page doesn't reset the clock)
  // --------------------------------------------------
  useEffect(() => {
  ...
  useEffect(() => {
    async function checkTableStatus() {
      ...
      if (data) {
        let allowed = data.status === 'occupied'

        // Safety net: auto-expire if occupied too long (staff forgot to close it)
        if (allowed && data.occupied_since) {
          const hoursOpen =
            (Date.now() - new Date(data.occupied_since).getTime()) / (1000 * 60 * 60)
